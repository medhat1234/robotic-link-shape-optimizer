...
void selectActuation(double Arm_mass_kg, double Link_length_mm, double Paylaod_mass_kg, double Angular_acceleration, double Angular_velocity_req ,const vector<Motor>& motors, const vector<Gearbox>& gearboxes) {
    ...
    double Torque_req = (Arm_mass_kg * G * (Link_length_m / 2.0)) + (Paylaod_mass_kg * G * Link_length_m) + (Arm_mass_kg * pow(Link_length_m / 2.0, 2) * Angular_acceleration)
                        + (Paylaod_mass_kg * pow(Link_length_m, 2) * Angular_acceleration);
    // compareing between motors and gearboxes.
    double bestCost = 1e18; // limit of the cost .
    string bestCombo ;
    // by using range based for loops.
    for (const auto &motor : motors) {
        for (const auto &gearbox : gearboxes) {
            double Torque_out = motor.torque * gearbox.ratio * gearbox.efficiency;
            //the condition checks if the motor-gearbox combo can meet both the torque and speed requirements.
            if (Torque_out >= Torque_req && (motor.speed / gearbox.ratio) >= ((Angular_velocity_req*60)/(2*PI)) ){
...
int main() {
    ...
    double L, mp, alpha, angular_velocity_req, d1, d2 = 0;
    ...
    cout << "Required Angular Velocity (rad/s): "; cin >> angular_velocity_req;
    ...
    selectActuation(ml, L, mp, alpha, angular_velocity_req, motors, gearboxes);
